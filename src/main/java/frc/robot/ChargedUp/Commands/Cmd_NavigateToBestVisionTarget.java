...
public class Cmd_NavigateToBestVisionTarget extends CommandBase {  
   ...
   *  <h4> NOTE: IF you choose to use enableY and enableRotation at the same time, unexpected results may occur
   */
  public Cmd_NavigateToBestVisionTarget(SubSys_DriveTrain subSys_DriveTrain, SubSys_Photonvision subSys_Photonvision, SubSys_Bling subSys_Bling, PhotonCamera camera, int pipelineIndex, boolean enableX, boolean enableY, boolean enableRotation) {
  ...
  public void execute() {
    ...
    switch(pipelineIndex){
      case Const_Photonvision.Pipelines.Cube: acceptableDistanceToTarget = Const_Photonvision.TargetingConstants.Cube.GOAL_RANGE_METERS; break;
      case Const_Photonvision.Pipelines.Cone: acceptableDistanceToTarget = Const_Photonvision.TargetingConstants.Cone.GOAL_RANGE_METERS; break;
      case Const_Photonvision.Pipelines.Apriltag: acceptableDistanceToTarget = Const_Photonvision.TargetingConstants.GridApriltag.GOAL_RANGE_METERS; break;
    }
  ...
  public boolean isFinished() {
    ...
    if (enableX) {
      xFinished = (Xcontroller.getPositionError() < AcceptablePIDError.X_PID_Error && Xcontroller.getPositionError() > -AcceptablePIDError.X_PID_Error);
    }
    if (enableY) {
      yFinished = (Ycontroller.getPositionError() < AcceptablePIDError.Y_PID_Error && Ycontroller.getPositionError() > -AcceptablePIDError.Y_PID_Error);
    }
    if (enableRotation) {
  ...
  private double getVisionStrafeSpeed(PhotonPipelineResult result){
    if (result.hasTargets()) {
      return Math.min(Math.max(Ycontroller.calculate((result.getBestTarget().getYaw()), 0), -PIDspeeds.Max_Y_PID_Speed), PIDspeeds.Max_Y_PID_Speed);
    } else {
      return 0;
    }
  }
