/**
 * A command that closes the hand when a target is detected within range using a PhotonVision camera.
 *
 * <p>This command uses a PhotonVision camera and a specified pipeline to detect a target. When the target
 * is detected within range, the hand is closed.
 *
 */
...
public class Cmd_CloseHandInProximity extends CommandBase {
  ...
  /**
   * Constructs a new {@link Cmd_CloseHandInProximity} command with the specified PhotonVision camera, pipeline index, 
   * and hand subsystem.
   *
   ...
   */
  public Cmd_CloseHandInProximity(SubSys_Photonvision subSys_Photonvision, SubSys_Hand subSys_Hand, PhotonCamera camera, int pipelineIndex) {
