...
public class MK4i_FalconFalcon_Module {
  ...
  public void setDesiredState(SwerveModuleState desiredState, boolean isOpenLoop) {
    ...
    if (isOpenLoop) {
      double percentOutput =
          desiredState.speedMetersPerSecond / Constants.RobotSettings.DriveTrain.DriveTrainMaxSpd;
      driveMotor.set(ControlMode.PercentOutput, percentOutput);
    }
    // Closed Loop
    else {
      double velocity =
          TalonFX_Conversions.MPSToFalcon(
    ...
    if (Math.abs(desiredState.speedMetersPerSecond)
        > (Constants.RobotSettings.DriveTrain.DriveTrainMaxSpd * 0.01)) {
      angle = desiredState.angle.getDegrees();
    }

    // Set Steet Motor Command to angle
    steerMotor.set(ControlMode.Position, TalonFX_Conversions.degreesToTalonFX(angle));
  ...
  public SwerveModuleState getState() {
    ...
    double velocity =
        TalonFX_Conversions.falconToMPS(
  ...
  public SwerveModulePosition getPosition() {
    ...
    swrModulePosition = new SwerveModulePosition(getDriveMotorDistance(), getSteerAngle());
  ...
  /**
   * getDriveMotorPosition Get Drive Motor Sensor Position
   *
  ...
  /**
   * getDriveWheelRevs Get Drive Motor
   *
  ...
  public double getDriveWheelRevs() {
    ...
    double wheelRevs =
        TalonFX_Conversions.talonFXToRevs_GearRatio(
  ...
  /**
   * getSteerMotorPos Return Steer Motor Position
   *
